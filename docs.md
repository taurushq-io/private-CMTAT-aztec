...
- **Current Status**: Solutions are still under consideration, and encryption mechanisms are being actively developed.
...
- **Issuer's View of User Balances**:
  - We desire for the issuer to view the private balance of the user at any time.
  - **Potential Solutions**:
    - Use a special note shared between the user and issuer with a shared viewing key.
    - Challenges include key management and differentiating these notes from standard notes.

- **Force Transfer Requirement**:
...
- **Note Encryption in PXE**:
  ...
  - Notes are not yet encrypted in the user's PXE. There should be one PXE per user, which can have multiple accounts.
...
- **Duplicate Note Emission**:
  - If we emit the same note twice, one for the user and one for the issuer, what do we use instead of the `oak_m` of the second encryption?
...
- **Key Rotation**: Implement key rotation following [Aztec's Common Patterns](https://docs.aztec.network/guides/smart_contracts/writing_contracts/common_patterns/key_rotation).
...
- **Other Platforms**:
  ...
  - **Silent Data**: (Further information needed)
